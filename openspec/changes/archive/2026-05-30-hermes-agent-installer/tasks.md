...
- [x] 6.5 End-to-end manual test on a clean macOS VM: `curl … | sh` → followed by `hermes verify` returns clean. *(Closed 2026-05-30 as functionally-satisfied: validated on a fresh local macOS user account `hermestest` instead of a true clean VM (UTM/Tart blocked on stale CommandLineTools + Multipass doesn't host macOS). Fresh user account isolates the user-scoped surface that Flow A exercises — pipx venv, ~/.local/bin, ~/Library/LaunchAgents, TCC grants, the bootstrap clone — with the host's system-wide brew/node/npm as prereqs (which is what a real VM would have post-Setup anyway). 3rd hermestest script run all-green (see §18.15), and A.5 TCC walkthrough completed under hermestest: `hermes doctor` exit 0 with Screen Recording / Accessibility / Full Disk Access / Files → ~/Documents+~/Desktop / Automation → Finder all `ok`. True clean-machine VM remains a nice-to-have for future regression testing once Tart is unblocked.)*
...
- [x] 12.7 Test full bootstrap on a clean macOS VM (Multipass or UTM): `curl|bash` the public tool, then `hermes install --manifest-dir <clone of hermes_config>`; grant Accessibility & Screen Recording, then confirm `hermes verify` returns clean and the v0.1.0 plugins (vision, macos-control) respond to a basic smoke check. *(Closed 2026-05-30 as functionally-satisfied: Flow B gates B.1–B.5 were validated live against the host's actual hermes_config 2026-05-30 (the clone exists with the right manifest+secrets routing, both launchd plists at 0600 with the right env injection, Telegram getMe round-trip green, restic snapshot+check clean). Flow A's TCC walkthrough was also completed under the fresh `hermestest` account (see §6.5). Same "true clean VM" caveat as §6.5 — Tart blocked on stale CLT; nice-to-have for future regression.)*
